...
def searchForPlayer(targetRect, searchRect, direction, Surface, scroll, obstacles,  maxAngle=40, maxSearchUnits=24):
    ...
    dirVar = KDS.Math.Jd(direction)*-1
...
class LegacyHostileEnemy:
    def __init__(self, _health:int, _speed:int, _position:(int,int),__tilerects:list, _animation_name: str):
        self.health = _health
        self.speed = -_speed
...
class LegacyImp(LegacyHostileEnemy):
