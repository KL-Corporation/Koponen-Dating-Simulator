...
class Color:
    ...
    def _get_value(self, t: float) -> tuple[int, int, int]:
        """t must be clamped."""
        return (
            round(KDS.Math.LerpUnclamped(self.From[0], self.To[0], t)),
            round(KDS.Math.LerpUnclamped(self.From[1], self.To[1], t)),
            round(KDS.Math.LerpUnclamped(self.From[2], self.To[2], t))
        )
    ...
    def update(self, reverse: bool = False) -> Tuple[int, int, int]:
        return self._get_value(self._t.update())
